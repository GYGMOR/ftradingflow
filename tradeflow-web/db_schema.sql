-- TradeFlow PostgreSQL Database Schema
-- Standard: snake_case
...
CREATE TABLE entitlements (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    code TEXT UNIQUE NOT NULL, -- e.g., 'WIDGET_AI_RADAR', 'SIDEBAR_RISK_CENTER'
    name TEXT NOT NULL,
    description TEXT
);
...
CREATE TABLE user_purchases (
    ...
    payment_provider_id TEXT, -- e.g., Stripe Payment Intent ID
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE user_dashboard_layouts (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID REFERENCES users(id),
    scene_name TEXT NOT NULL, -- 'classic', 'scalper', 'ai'
    layout_data JSONB NOT NULL, -- react-grid-layout full state
...
CREATE TABLE user_active_widgets (
    user_id UUID REFERENCES users(id),
    widget_id TEXT NOT NULL, -- e.g., 'ai_radar'
...
CREATE TABLE exchange_connections (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID REFERENCES users(id),
    exchange_name TEXT NOT NULL, -- 'binance', 'kraken'
